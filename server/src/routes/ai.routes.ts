...
const DEFAULT_TENANT = 'antigravity-tenant-id'; // Using local tenant ID

// POST /api/ai/score/:leadId
router.post('/score/:leadId', async (req: any, res) => {
    try {
        const lead = await prisma.lead.findUnique({
            where: { id: req.params.leadId, tenantId: DEFAULT_TENANT }
        });
...
router.post('/draft', async (req: any, res) => {
    try {
        ...
        const lead = await prisma.lead.findUnique({
            where: { id: leadId, tenantId: DEFAULT_TENANT }
        });
